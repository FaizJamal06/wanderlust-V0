...
app.use(express.json()); // <--- Crucial for parsing Next.js JSON API requests
app.use(methodOverride("_method"));

// CORS — allow the Next.js frontend (dev: 3000, prod: your domain)
app.use(cors({
  origin: [
    'http://localhost:3000',
    'http://localhost:3001',
...
// JSON API routes (consumed by the Next.js immersive frontend)
...
app.use((err, req, res, next) => {
  ...
  // Return JSON for API routes instead of rendering EJS
...
// Keep-alive pinger (Self-pinging to prevent Render from sleeping)
if (process.env.RENDER_EXTERNAL_HOSTNAME) {
  const https = require("https");
  const hostname = process.env.RENDER_EXTERNAL_HOSTNAME;
  setInterval(() => {
    https.get(`https://${hostname}/api/health`, (res) => {
      console.log(`Self-ping sent to https://${hostname}/api/health - Status: ${res.statusCode}`);
    }).on('error', (err) => {
